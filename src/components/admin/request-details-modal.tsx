...
export function RequestDetailsModal({ open, onClose, request: initialRequest }: RequestDetailsModalProps) {
  ...
  const isRequestInPast = request.startDate?.seconds 
    ? new Date(request.startDate.seconds * 1000) < new Date()
    : false;
      ...
      <div className="p-6">
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-x-8 gap-y-4">
            ...
            {request.type === 'days_off' && (
              <div>
                <p className="text-sm font-medium text-gray-500">Days Off</p>
                <p className="mt-1 text-base text-gray-900">{request.daysOff || 0}</p>
              </div>
            )}

            {request.type === 'hours_off' && (
              <div>
                <p className="text-sm font-medium text-gray-500">Hours Off</p>
                <p className="mt-1 text-base text-gray-900">{request.hoursOff || 0}</p>
              </div>
            )}
          </div>
